...
#
# Tags look like `v<GoToSocial version>-<release>`:
#
# - if defaults/main.yml points at a GoToSocial version that has never been
#   released, the release counter restarts at 0 (`v0.22.1-0`)
# - otherwise the counter is incremented (`v0.22.1-1`), but only if something
...
version="$(sed -nE 's|^gotosocial_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the GoToSocial version from $defaults_path"
	exit 1
fi

# The version values do not carry a leading `v` (e.g. `0.22.1`), but the tags
# do (`v0.22.1-0`). Stripping any `v` before prepending one keeps this correct
